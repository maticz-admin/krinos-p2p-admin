...
const Sidebar = (props) => {
  ...
  // }
  useEffect(() => {
    document.getElementsByTagName("body")[0].classList.remove("shrink_whole");
  }, []);
  ...
  return (
    <div>
      <button className="btn" onClick={changestate} id="menu-toggle">
                  ...
                  nav.map((item, i) => {
                    if (item.sidemenu) {
                      let filterData =
                        ...
                        restriction.find((item) => item == "/dashboard");

                      if (!filterData) {
                        restriction &&
                          restriction.length > 0 &&
                          restriction.push("/dashboard");
                      }
                      if (
                            ...
                            {item && item.childItem ? (
                              <>
                                <a
                                  ...
                                  aria-expanded="false"
                                  className="list-group-item list-group-item-action flex-column align-items-start list_grp_active"
                                >
